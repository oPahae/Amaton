...
export default function Forgot() {
  ...
  const handleNext = async () => {
    ...
    if (step === 0) {
      ...
      try {
        const response = await axios.post("http://localhost:5000/api/auth/sendCode", { email });
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/auth/verifyCode", { email, code: code.join("") });
  ...
  const handleSavePassword = async () => {
    ...
    try {
      await axios.post("http://localhost:5000/api/auth/resetPassword", { email, password: password.newPassword });
