...
export default function Forgot() {
  ...
  const handleNext = async () => {
    ...
    if (step === 0) {
      ...
      try {
        const response = await axios.post("http://localhost:5000/api/admin/sendCode")
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/admin/verifyCode", { code: code.join("") })
  ...
  const handleSavePassword = async () => {
    ...
    try {
      await axios.post("http://localhost:5000/api/admin/resetPassword", { password: password.newPassword })
      ...
      <div className="relative w-full max-w-md bg-white p-8 rounded-xl shadow-lg overflow-hidden">
        ...
        <div ref={formRef} className="relative">
          ...
          {step === 0 && (
            <>
              ...
              <button
                ...
                onClick={handleNext}
              >
                {!sendCodeLoading ? "Next" : <div className="flex justify-center"><div className="w-8 h-8 border-4 border-white border-dashed rounded-full animate-spin"></div></div>}
              </button>
            </>
          )}
          ...
          <div className="text-center mt-4">
            <Link href="/Admin/Login" className="text-violet-600 hover:underline">
              Back
            </Link>
          </div>
        </div>
